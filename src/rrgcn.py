...
class RGCNCell(BaseRGCN):
    ...
    def forward(self, g, init_ent_emb, init_rel_emb):
        if (
            self.encoder_name == "uvrgcn"
            or self.encoder_name == "kbat"
            or self.encoder_name == "compgcn"
        ):
...
class RecurrentRGCN(nn.Module):
    def __init__(
        ...
        self.emb_rel = torch.nn.Parameter(
            torch.Tensor(self.num_rels * 2, self.h_dim), requires_grad=True
        ).float()
        torch.nn.init.xavier_normal_(self.emb_rel)

        self.dynamic_emb = torch.nn.Parameter(
            torch.Tensor(num_ents, h_dim), requires_grad=True
        ).float()
        torch.nn.init.normal_(self.dynamic_emb)

        if self.use_static:
            self.words_emb = torch.nn.Parameter(
                torch.Tensor(self.num_words, h_dim), requires_grad=True
            ).float()
        ...
        self.rgat_layer = RGAT(
            self.h_dim, self.h_dim, activation=F.rrelu, dropout=dropout, self_loop=True
        )
        self.projection_model = MLPLinear(self.h_dim, self.h_dim)

        self.time_gate_weight = nn.Parameter(torch.Tensor(h_dim, h_dim))
        nn.init.xavier_uniform_(
            self.time_gate_weight, gain=nn.init.calculate_gain("relu")
        )
        ...
        nn.init.xavier_uniform_(
            self.pre_gate_weight, gain=nn.init.calculate_gain("relu")
        )
    ...
    def forward(self, sub_graph, T_idx, query_mask, g_list, static_graph, use_cuda):
        ...
        if self.use_static:
            ...
            self.h = (
                F.normalize(self.dynamic_emb)
                if self.layer_norm
                else self.dynamic_emb[:, :]
            )
            static_emb = None
        ...
        if self.pre_type == "all":
            for i, g in enumerate(g_list):
                ...
                )  # 论文中的公式(10)，所以w2是论文中的W5，att_e就是论文中的ɑi
                ...
                time_weight = F.sigmoid(
                    torch.mm(x_input, self.time_gate_weight) + self.time_gate_bias
                )  # 论文中的公式(8)，所以time_gate_weight是论文中的W3
                self.hr = (
                    time_weight * x_input + (1 - time_weight) * self.emb_rel
                )  # 论文中的公式(7)
                self.hr = F.normalize(self.hr) if self.layer_norm else self.hr
    ...
    def predict(
        ...
        with torch.no_grad():
            ...
            query_mask = (
                torch.zeros((self.num_ents, self.h_dim)).to(self.gpu)
                if use_cuda
                else torch.zeros(1)
            )
            ...
            embedding, _, r_emb, his_emb, his_r_emb, _, _ = self.forward(
                sub_graph, T_id, query_mask, test_graph, static_graph, use_cuda
            )
    ...
    def get_loss(
        self, que_pair, sub_graph, T_idx, glist, triples, static_graph, use_cuda
    ):
        """
        :param glist:
        :param triplets:
        ...
        query_emb = self.w1(
            torch.concat([e1_emb, rel_emb], dim=1)
        )  # 论文中公式(4)的前半部分
        query_mask[uniq_e] = query_emb
        ...
        (
            ...
            his_temp_embs,
            his_rel_embs,
        ) = self.forward(sub_graph, T_idx, query_mask, glist, static_graph, use_cuda)

        if self.pre_type == "all":
            scores_ob, _ = self.decoder_ob.forward(
                embedding, r_emb, all_triples, his_emb, self.pre_weight, self.pre_type
            )
            score_seq = F.softmax(scores_ob, dim=1)
            score_en = score_seq
        ...
        if self.relation_prediction:
            score_rel = self.rdecoder.forward(
                embedding, r_emb, all_triples, mode="train"
        ...
        if self.use_cl and self.pre_type == "all":
            for id, evolve_emb in enumerate(his_temp_embs):
                ...
                query = torch.concat(
                    [self.his_ent[all_triples[:, 0]], his_r_emb[all_triples[:, 1]]],
                    dim=1,
                )
                query2 = torch.concat(
                    [
                        evolve_emb[all_triples[:, 0]],
                        his_rel_embs[id][all_triples[:, 1]],
                    ],
                    dim=1,
                )
                x1 = self.w_cl(query)
