...
def get_sample_from_history_graph3(
    subg_arr, sr_to_sro, triples, num_nodes, num_rels, use_cuda, gpu
):
    ...
    subg_df = (
        df.groupby(df.columns.tolist()).size().reset_index().rename(columns={0: "freq"})
    )
...
def test(
    ...
    if mode == "test":
        ...
        print(
            f"Load Model name: {model_name}. Using best epoch : {checkpoint['epoch']}"
    ...
    sr_to_sro = np.load(
        f"./data/{args.dataset}/his_dict/train_s_r.npy", allow_pickle=True
    ).item()
    ...
    for time_idx, test_snap in enumerate(tqdm(test_list)):
        ...
        test_triples_input_inv = (
            torch.LongTensor(inverse_triples).cuda()
            if use_cuda
            else torch.LongTensor(inverse_triples)
        )
        ...
        mrr_filter_snap_inv, mrr_snap_inv, rank_raw_inv, rank_filter_inv = (
    ...
    print(
        f"(all_raw) MRR, Hits@ (1,3,5):{all_mrr_raw.item():.6f}, {all_hit_raw[0]}, {all_hit_raw[1]}, {all_hit_raw[2]}"
    )
    print(
        f"(all_filter) MRR, Hits@ (1,3,5):{all_mrr_filter.item():.6f}, {all_hit_filter[0]}, {all_hit_filter[1]}, {all_hit_filter[2]}"
    )
...
def run_experiment(args, n_hidden=None, n_layers=None, dropout=None, n_bases=None):
    ...
    model_name = f"{args.dataset}-len{args.train_history_len}-gpu{args.gpu}-lr{args.lr}-{args.temperature}-{args.pre_weight}-{args.use_cl}-{args.pre_type}-{args.n_hidden}-{args.encoder}-{time.time()}"
    model_state_file = f"./models/{model_name}.pt"
    print(f"Sanity Check: stat name : {model_state_file}")
    print(f"Sanity Check: Is cuda available ? {torch.cuda.is_available()}")
    ...
    if args.add_static_graph:
        ...
        static_node_id = (
            torch.from_numpy(np.arange(num_words + data.num_nodes))
            .view(-1, 1)
            .long()
            .cuda(args.gpu)
            if use_cuda
            else torch.from_numpy(np.arange(num_words + data.num_nodes))
            .view(-1, 1)
            .long()
        )
    else:
    ...
    if args.test and os.path.exists(model_state_file):
        ...
        print(
            f"--------------{model_state_file} not exist, Change mode to train and generate stat for testing----------------\n"
        )
    else:
        print(
            "----------------------------------------start training----------------------------------------\n"
        ...
        for epoch in range(args.n_epochs):
            ...
            for train_sample_num in tqdm(idx):
                ...
                subgraph_arr = np.load(
                    f"./data/{args.dataset}/his_graph_for/train_s_r_{train_sample_num}.npy"
                )
                subgraph_arr_inv = np.load(
                    f"./data/{args.dataset}/his_graph_inv/train_o_r_{train_sample_num}.npy"
                )
                subg_snap = build_graph(
                    num_nodes, num_rels, subgraph_arr, use_cuda, args.gpu
                )  # 取出采样子图
                subg_snap_inv = build_graph(
                    num_nodes, num_rels, subgraph_arr_inv, use_cuda, args.gpu
                )
                ...
                # break
            print(
                f"Epoch {epoch:04d} | Ave Loss: {np.mean(losses):.4f} | entity-relation-static:{np.mean(losses_e):.4f}-{np.mean(losses_r):.4f}-{np.mean(losses_static):.4f} Best MRR {best_mrr:.4f} | Model {model_name}"
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-d", "--dataset", type=str, default="GDELT", help="dataset to use"
    )
    parser.add_argument(
        "--test",
        action="store_true",
    ...
    parser.add_argument(
        "--run-analysis", action="store_true", default=False, help="print log info"
    )
    parser.add_argument(
        "--run-statistic",
        action="store_true",
    ...
    parser.add_argument(
        "--pre-type", type=str, default="short", help=["long", "short", "all"]
    )
    parser.add_argument(
        "--use-cl",
        action="store_true",
    ...
    parser.add_argument(
        "--temperature", type=float, default=0.07, help="the temperature of cl"
    )
    ...
    parser.add_argument(
        "--encoder",
        type=str,
        default="uvrgcn",  # {uvrgcn,kbat,compgcn}
        help="method of encoder",
    )
    parser.add_argument("--opn", type=str, default="sub", help="opn of compgcn")
    parser.add_argument(
        "--aggregation", type=str, default="none", help="method of aggregation"
    )
    parser.add_argument(
